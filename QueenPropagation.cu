...
struct QueenPropagation{
	...
	__device__ QueenPropagation(VariableCollection& vars,int,int);	//initialize queue
	...
	__device__ ~QueenPropagation();				//free for lastValues

};

////////////////////////////////////////////////////////////////////////////

__device__ QueenPropagation::QueenPropagation(VariableCollection& vars, int nvr, int nvl):
		vc(vars),nVars(nvr),nVals(nvl),queue(nVars,nVals){
	lastValues = (int*)malloc(nVars*sizeof(int));
